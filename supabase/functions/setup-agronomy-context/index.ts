...
function requiredNumber(source: UnknownRecord, key: string): number {
  const value = source[key];
...
const handler = withSupabase({ auth: "user" }, async (req, ctx) => {
  ...
  try {
    ...
    const queuedAtLocal = requiredString(body, "queuedAtLocal");
    ...
    if (!isRecord(body.establishment) || !isRecord(body.field) || !isRecord(body.campaign)) {
      return json(
        {
          clientOperationId,
          status: "rejected",
    ...
    const { data, error } = await ctx.supabaseAdmin.rpc(
      "process_setup_agronomy_context",
      {
        ...
        p_establishment_id: requiredString(body.establishment, "id"),
        p_establishment_name: requiredString(body.establishment, "name"),
        p_field_id: requiredString(body.field, "id"),
        p_field_name: requiredString(body.field, "name"),
        p_nominal_area_ha: requiredNumber(body.field, "nominalAreaHa"),
        p_campaign_id: requiredString(body.campaign, "id"),
        p_campaign_name: requiredString(body.campaign, "name"),
        p_campaign_starts_on: requiredString(body.campaign, "startsOn"),
        p_campaign_ends_on: requiredString(body.campaign, "endsOn"),
    ...
    if (error) {
      ...
      console.error("setup-agronomy-context database error", {
        code,
